...
pub struct ResolutionEngine {
    resolution_config: super::config::ResolutionConfig,
    interpolation_config: super::config::InterpolationConfig,
...
impl ResolutionEngine {
    pub fn new(
        ...
        Self {
            resolution_config: resolution.clone(),
            interpolation_config: interpolation.clone(),
    ...
    /// Calculate combined version from all snapshots.
    ...
    /// Rebuild dependency graph only if versions changed.
    ...
    /// Filter snapshots by source IDs, returning references instead of clones.
        ...
        match file_source_filter {
            Some(filter) if !filter.is_empty() => snapshots
                ...
                .filter(|snapshot| {
                    ...
                    let is_file_source = source_id_str.starts_with("file:") && source_id_str.len() > 5;
                    if is_file_source {
    ...
    fn all_variables_inner(
        ...
        let precedence = self.resolution_config.precedence.clone();
        let mut seen_keys = std::collections::HashSet::new();
        let mut results = Vec::new();

        for _precedence_item in &precedence {
    ...
    fn resolve_variable(
        ...
        let max_depth = self.interpolation_config.max_depth;

        if !self.interpolation_config.enabled {
    ...
    /// Lazy interpolation that only resolves referenced variables.
    fn interpolate_value_lazy(
        ...
        if depth >= max_depth || !self.interpolation_config.enabled {
        ...
        // Only add variables that are actually referenced in this value
    ...
    fn build_dependency_graph(&self, snapshots: &[crate::source::SourceSnapshot]) -> Result<()> {
        ...
        // Build edges by finding variable references in values
        for snapshot in snapshots {
            for variable in snapshot.variables.iter() {
                ...
                for ref_key in references {
                    graph.add_edge(
                        variable.key.clone(),
                        ref_key,
                        Some((0, 0)), // Line/col would require korni span info
                    );
                }
            }
        }
